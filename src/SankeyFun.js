...
export default function SankeyFun (
  ...
  nodes.forEach((element) => {
    if (element.id.includes('1.')) {
      ...
      nodeColors[element.index] = greyColor.copy({opacity: 0.2});
    } else {
  ...
  const node = svg
    ...
    .attr("x", (d) => d.x0)
    .attr("y", (d) => d.id.includes('pass') ? d.y0 + 2 : d.y0)
    .attr("height", (d) => d.id.includes('pass') ? Math.max(4, d.y1 - d.y0 - 4) : Math.max(4, d.y1 - d.y0))
  ...
  if (Tl) {
    svg
      ...
      .text(({ index: i }) =>  {
        ...
        return String(Tl[i]).includes("col") & tail((String(Tl[i]).split(' '))) !== 'null' & !tail((String(Tl[i]).split(' '))).includes('pass')
          ? tail((String(Tl[i]).split(' '))) : ""; // String(Tl[i]).replace("col", '') : tail((String(Tl[i]).split(' ')));
      });
