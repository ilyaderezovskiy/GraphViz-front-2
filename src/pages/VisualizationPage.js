...
function VisualizationPage() {
  ...
  return (
    <div>
        ...
        <Box m={0} pb={0} position={'fixed'} top={0} right={115} display="flex" justifyContent="space-between"
        ...
        <FormGroup>
          ...
          sx={{
            ...
            />} label= {<Typography variant="h8" color={'white'}>Первая подгруппа</Typography>} 
          />
          ...
          sx={{
            ...
            />} label= {<Typography variant="h8" color={'white'}>Вторая подгруппа</Typography>} 
            />

        </FormGroup>
        <Button variant="contained" sx={{mt:1.5}} onClick={() => {
          if (energy.length !== 0 && verticesOrder.find(x => x.id === energy.id + 'v')) {
            filterData(getFilterData())
          }}}>Применить
        </Button>
